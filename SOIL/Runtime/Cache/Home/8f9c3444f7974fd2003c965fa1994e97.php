...
<div class="left-style" align="center">
    ...
    <div class="content main-div" style="height:auto">
		...
		<a href="<?php echo U('Type/addtype');?>">
		<button type="button" class="btn btn-primary btn-sm">新增组</button>
		</a>
		</div>
		...
		<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td>
			<?php if($vo["state"] == '1' ): ?>启动
			<?php else: ?>
				停止<?php endif; ?>
			</td>
			<td>
				<a href="<?php echo U('Type/addactionbytype',array('id'=>$vo['id']));?>">
				<button type="button" class="btn btn-success btn-xs">行为分配</button>
				</a>
			</td>
			<td>
				<?php if($vo["state"] == '1' ): ?><a href="<?php echo U('Type/typestate',array('var'=>'0','id'=>$vo['id']));?>">
				<button type="button" class="btn btn-danger btn-xs">停用</button>
				</a>
				<?php else: ?>
				<a href="<?php echo U('Type/typestate',array('var'=>'1','id'=>$vo['id']));?>">
				<button type="button" class="btn btn-success btn-xs">启用</button>
				</a><?php endif; ?>
				<a href="<?php echo U('Type/addtype',array('state'=>'detail','id'=>$vo['id']));?>">
