...
<div class="left-style" align="center">
    ...
    <div class="content main-div" style="height:auto">
    	<ol class="breadcrumb col-lg-12">
		  ...
		  <li><a href="<?php echo U('Type/userlist');?>">用户组管理</a></li>
		  <li class="active">权限组分配</li>
		</ol>
		<h4><strong><span class="label label-primary"><?php echo ($data[0]['name']); ?></span>目前拥有权限组</strong></h4>
		<table class="table table-striped table-bordered" style="width: 680px;">
			<?php if($data[0]['replenish'] == ''): ?><tr>
				<th>
					<span class="label label-default">暂无任何授权权限</span>
				</th>
			</tr>
				<?php else: ?>
			<tr>
				<th><?php echo ($data[0]['replenish']); ?></th>
			</tr><?php endif; ?>
		</table>
		<h4><strong>选择权限组</strong></h4>
		<div class="content">
			<table class="table">
			<tr>
				<td>权限名</td>
			...
			<?php if(is_array($var)): $i = 0; $__LIST__ = $var;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
					<td>
						<strong><?php echo ($vo["typename"]); ?></strong>		
					</td>
					<td>
						<?php echo ($vo["dis"]); ?>
					</td>
					<td>
					<?php if($vo["state"] == 1): ?><span class="label label-primary">启用</span>
						<?php else: ?>
						<span class="label label-default">废弃</span><?php endif; ?>
					</td>
					<td align="center">
						<?php if($vo["state"] == 1): if(in_array(($vo["typename"]), is_array($data[0]['replenish'])?$data[0]['replenish']:explode(',',$data[0]['replenish']))): ?><a href="<?php echo U('Type/usertype',array('typenames'=>$vo['typename'],'name'=>$data[0]['name']));?>">
							<button type="button" class="btn btn-danger btn-xs">离权</button>
							</a>
						<?php else: ?>
							<a href="<?php echo U('Type/usertype',array('typenamee'=>$vo['typename'],'name'=>$data[0]['name']));?>">
							<button type="button" class="btn btn-primary btn-xs">授权</button>
							</a><?php endif; ?>
						<?php else: ?>
							<span class="label label-default">权限组停用</span><?php endif; ?>
		...
		<hr>
		<div align="right">
			<a href="<?php echo U('Type/userlist');?>">
			<button type="button" class="btn btn-primary btn-sm" name="ajax" id="ajax">返回</button>
			</a>
		</div>
    <!--body-->
