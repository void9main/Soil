...
<div class="left-style" align="center">
    ...
    <div class="content main-div" style="height:auto;width: 50%;">
    	<ol class="breadcrumb">
		  ...
		  <li><a href="<?php echo U('Type/typelist');?>">权限组管理</a></li>
		  <li class="active">行为分配</li>
		</ol>
    	<h4><span class="label label-success"><?php echo ($val[0]['typename']); ?></span>当前行为策略:</h4>
    		<br />
	    	<?php if(is_array($var)): $i = 0; $__LIST__ = $var;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><span class="label label-primary"><?php echo ($vo); ?></span>
	    		&nbsp;&nbsp;<?php endforeach; endif; else: echo "" ;endif; ?>
    	<br /><br />
    	<h4>所有行为策略表</h4>
	    	...
	    	<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
	    				...
	    				<td>
	    					<?php if($vo["state"] == '1' ): ?><span class="label label-primary">启动</span>
							<?php else: ?>
								<span class="label label-default">停止</span><?php endif; ?>
	    				</td>
	    				<td>
	    					<?php if($vo["state"] == '1' ): if(in_array(($vo["module"]), is_array($var)?$var:explode(',',$var))): ?><a href="<?php echo U('Type/addactionbytype',array('ids'=>$vo['id'],'name'=>$val[0]['typename'],'module'=>$vo['module']));?>">
		    						<button type="button" class="btn btn-danger btn-xs">移除</button>
		    					</a>
		    				<?php else: ?>
		    					<a href="<?php echo U('Type/addactionbytype',array('ide'=>$vo['id'],'name'=>$val[0]['typename'],'module'=>$vo['module']));?>">
		    						<button type="button" class="btn btn-primary btn-xs">添加</button>
		    					</a><?php endif; ?>
	    					<?php else: ?>
	    						<span class="label label-default">策略终止</span><?php endif; ?>
	    				</td>
